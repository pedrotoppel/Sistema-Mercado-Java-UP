...
public class ProdutoAlimenticioView {

    private final ProdutoAlimenticioController controller;
    private final CategoriaController categoriaController;
    private final FornecedorController fornecedorController;
    private final Leitura leitura;

    public ProdutoAlimenticioView(Scanner scanner, ProdutoAlimenticioController controller,
    ...
    private void cadastrar() {
        ...
        String validade = leitura.texto("Data de validade (dd/mm/aaaa): ");
        try {
            ProdutoAlimenticio p = controller.cadastrar(nome, preco, qtd, categoria, fornecedor, validade);
    ...
    private void listar() {
        List<ProdutoAlimenticio> produtos = controller.listar();
        if (produtos.isEmpty()) {
            System.out.println("Nenhum produto cadastrado.");
            return;
        }
        for (ProdutoAlimenticio p : produtos) {
            System.out.println(p + " | validade: " + p.getDataValidade());
        }
    }
    ...
    private void alterar() {
        ...
        Categoria categoria = selecionarCategoria();
        Fornecedor fornecedor = selecionarFornecedor();
        String validade = leitura.texto("Nova validade: ");
        try {
            boolean ok = controller.alterar(id, nome, preco, qtd, categoria, fornecedor, validade);
